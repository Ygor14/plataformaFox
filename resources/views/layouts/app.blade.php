...
<head>
    ...
    <!-- Meta tag que será atualizada pelo JavaScript -->
    <meta name="theme-color" id="dynamic-theme-color" content="#ffffff"> <!-- Cor temporária -->
    
    <!-- Script que pega a variável CSS e aplica -->
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Passo 1: Pega o valor REAL da variável CSS
            const root = document.documentElement;
            const cssColor = getComputedStyle(root)
                .getPropertyValue('--background-bottom-navigation-dark')
                .trim();
            
            // Passo 2: Atualiza a meta tag
            if (cssColor) {
                document.getElementById('dynamic-theme-color').content = cssColor;
            }
            
            // Opcional: Atualiza quando a cor mudar (ex: dark/light mode)
            new MutationObserver(() => {
                const updatedColor = getComputedStyle(root)
                    .getPropertyValue('--background-bottom-navigation-dark')
                    .trim();
                document.getElementById('dynamic-theme-color').content = updatedColor;
            }).observe(root, { 
                attributes: true, 
...
<body color-theme="dark" class="bg-base text-gray-800 dark:text-gray-300 ">
    ...
    localStorage.setItem('color-theme', 'dark'); // Define 'dark' como padrão
}
...
<script>
    document.documentElement.classList.add('dark'); // Sempre ativa o dark mode
    localStorage.setItem('color-theme', 'dark'); // Garante que fique salvo como dark

    // Remove qualquer possibilidade de modo claro
 ...
 @if (!empty($custom['custom_js']))
    ...
    @if(env('APP_ENV') === 'production')
        ...
        <!--<script>-->
        <!--    DisableDevtool({-->
                <!--clearLog: true, // Limpa o console-->
                <!--disableMenu: true, // Desativa o menu de clique direito-->
        <!--    });-->
        <!--</script>-->
    @endif
