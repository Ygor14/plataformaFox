...
class LimitSetting extends Page implements HasForms
{
    ...
    {
        return __('Configurações de Limites');
    }
        ...
        try {
            ...
            if ($this->record->update($this->data)) {
                ...
                Notification::make()
                    ...
                    ->body('Os limites foram configurados com sucesso!')
                    ->success()
                    ->send();
    ...
    public function form(Form $form): Form
    {
        return $form
            ->schema([
                Section::make('Configurações de Limites Financeiros')
                    ->description('Defina os limites de saque e seus períodos')
                    ->schema([
                        TextInput::make('withdrawal_limit')
                            ->label('Valor Máximo de Saque')
                            ->numeric()
                            ->required()
                            ->prefixIcon('heroicon-o-currency-dollar')
                            ->helperText('Valor máximo permitido para saques no período selecionado')
                            ->maxLength(191),
                            
                        Select::make('withdrawal_period')
                            ->label('Período de Limite')
                            ->options([
                                'daily' => 'Diário',
                                'weekly' => 'Semanal',
                                'monthly' => 'Mensal',
                            ...
                            ->required()
                            ->native(false)
                            ->helperText('Período para aplicação do limite de saque')
                    ])->columns(2)
